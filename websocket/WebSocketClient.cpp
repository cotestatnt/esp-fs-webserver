 ...
 * This file is part of the WebSocket for Arduino.
 *
...
/**
 * calles to init the WebSocket server
 */
...
void WebSocketClient::begin(String host, uint16_t port, String url, String protocol) {
    begin(host.c_str(), port, url.c_str(), protocol.c_str());
}

void WebSocketClient::begin(IPAddress host, uint16_t port, const char * url, const char * protocol) {
    return begin(host.toString().c_str(), port, url, protocol);
}

#if defined(HAS_SSL)
#if defined(SSL_AXTLS)
void WebSocketClient::beginSSL(const char * host, uint16_t port, const char * url, const char * fingerprint, const char * protocol) {
...
void WebSocketClient::beginSSL(String host, uint16_t port, String url, String fingerprint, String protocol) {
    beginSSL(host.c_str(), port, url.c_str(), fingerprint.c_str(), protocol.c_str());
}

void WebSocketClient::beginSslWithCA(const char * host, uint16_t port, const char * url, const char * CA_cert, const char * protocol) {
    begin(host, port, url, protocol);
    _client.isSSL = true;
    _fingerprint  = SSL_FINGERPRINT_NULL;
    _CA_cert      = CA_cert;
...
#else
void WebSocketClient::beginSSL(const char * host, uint16_t port, const char * url, const uint8_t * fingerprint, const char * protocol) {
    begin(host, port, url, protocol);
    _client.isSSL = true;
    _fingerprint  = fingerprint;
    _CA_cert      = NULL;
...
void WebSocketClient::beginSslWithCA(const char * host, uint16_t port, const char * url, BearSSL::X509List * CA_cert, const char * protocol) {
    begin(host, port, url, protocol);
    _client.isSSL = true;
    _fingerprint  = SSL_FINGERPRINT_NULL;
    _CA_cert      = CA_cert;
...
void WebSocketClient::setSSLClientCertKey(BearSSL::X509List * clientCert, BearSSL::PrivateKey * clientPrivateKey) {
    _client_cert = clientCert;
    _client_key  = clientPrivateKey;
}

void WebSocketClient::setSSLClientCertKey(const char * clientCert, const char * clientPrivateKey) {
...
void WebSocketClient::beginSocketIO(const char * host, uint16_t port, const char * url, const char * protocol) {
    begin(host, port, url, protocol);
    _client.isSocketIO = true;
}

void WebSocketClient::beginSocketIO(String host, uint16_t port, String url, String protocol) {
    beginSocketIO(host.c_str(), port, url.c_str(), protocol.c_str());
}

#if defined(HAS_SSL)
void WebSocketClient::beginSocketIOSSL(const char * host, uint16_t port, const char * url, const char * protocol) {
    begin(host, port, url, protocol);
    _client.isSocketIO = true;
    _client.isSSL      = true;
    _fingerprint       = SSL_FINGERPRINT_NULL;
...
void WebSocketClient::beginSocketIOSSL(String host, uint16_t port, String url, String protocol) {
    beginSocketIOSSL(host.c_str(), port, url.c_str(), protocol.c_str());
}

#if defined(SSL_BARESSL)
void WebSocketClient::beginSocketIOSSLWithCA(const char * host, uint16_t port, const char * url, BearSSL::X509List * CA_cert, const char * protocol) {
    begin(host, port, url, protocol);
    _client.isSocketIO = true;
    _client.isSSL      = true;
    _fingerprint       = SSL_FINGERPRINT_NULL;
...
void WebSocketClient::beginSocketIOSSLWithCA(const char * host, uint16_t port, const char * url, const char * CA_cert, const char * protocol) {
    begin(host, port, url, protocol);
    _client.isSocketIO = true;
    _client.isSSL      = true;
    _fingerprint       = SSL_FINGERPRINT_NULL;
 ...
 */
void WebSocketClient::loop(void) {
    if(_port == 0) {
        return;
    }
    WEBSOCKETS_YIELD();
 ...
 */
void WebSocketClient::onEvent(WebSocketClientEvent cbEvent) {
    _cbEvent = cbEvent;
}
 ...
 * @return true if ok
 */
bool WebSocketClient::sendTXT(uint8_t * payload, size_t length, bool headerToPayload) {
    if(length == 0) {
...
bool WebSocketClient::sendTXT(const uint8_t * payload, size_t length) {
    return sendTXT((uint8_t *)payload, length);
}

bool WebSocketClient::sendTXT(char * payload, size_t length, bool headerToPayload) {
...
bool WebSocketClient::sendTXT(String & payload) {
    return sendTXT((uint8_t *)payload.c_str(), payload.length());
}

bool WebSocketClient::sendTXT(char payload) {
 ...
 * @return true if ok
 */
bool WebSocketClient::sendBIN(uint8_t * payload, size_t length, bool headerToPayload) {
    if(clientIsConnected(&_client)) {
...
bool WebSocketClient::sendPing(String & payload) {
...
void WebSocketClient::disconnect(void) {
    if(clientIsConnected(&_client)) {
        WebSocket::clientDisconnect(&_client, 1000);
    }
}
 ...
 */
void WebSocketClient::setAuthorization(const char * user, const char * password) {
    if(user && password) {
        String auth = user;
        auth += ":";
        auth += password;
 ...
 */
void WebSocketClient::setAuthorization(const char * auth) {
    if(auth) {
 ...
 */
void WebSocketClient::setExtraHeaders(const char * extraHeaders) {
...
bool WebSocketClient::isConnected(void) {
 ...
 * @param length size_t
 */
void WebSocketClient::messageReceived(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin) {
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void WebSocketClient::clientDisconnect(WSclient_t * client) {
    bool event = false;
 ...
 */
bool WebSocketClient::clientIsConnected(WSclient_t * client) {
    if(!client->tcp) {
        return false;
    }
 ...
 */
void WebSocketClient::handleClientData(void) {
    ...
    if(len > 0) {
        switch(_client.status) {
            ...
            case WSC_CONNECTED:
                WebSocket::handleWebsocket(&_client);
                break;
            default:
                WebSocket::clientDisconnect(&_client, 1002);
                break;
        }
    }
    WEBSOCKETS_YIELD();
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void WebSocketClient::sendHeader(WSclient_t * client) {
...
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
    client->tcp->readStringUntil('\n', &(client->cHttpLine), std::bind(&WebSocketClient::handleHeader, this, client, &(client->cHttpLine)));
#endif
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void WebSocketClient::handleHeader(WSclient_t * client, String * headerLine) {
...
void WebSocketClient::connectedCb() {
    DEBUG_WEBSOCKETS("[WS-Client] connected to %s:%u.\n", _host.c_str(), _port);

#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
    _client.tcp->onDisconnect(std::bind([](WebSocketClient * c, AsyncTCPbuffer * obj, WSclient_t * client) -> bool {
...
void WebSocketClient::connectFailedCb() {
...
void WebSocketClient::asyncConnect() {
    ...
    tcpclient->onConnect(std::bind([](WebSocketClient * ws, AsyncClient * tcp) {
    ...
    tcpclient->onError(std::bind([](WebSocketClient * ws, AsyncClient * tcp) {
        ws->connectFailedCb();
 ...
 */
void WebSocketClient::handleHBPing() {
 ...
 */
void WebSocketClient::enableHeartbeat(uint32_t pingInterval, uint32_t pongTimeout, uint8_t disconnectTimeoutCount) {
    WebSocket::enableHeartbeat(&_client, pingInterval, pongTimeout, disconnectTimeoutCount);
}
