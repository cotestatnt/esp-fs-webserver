...
WebSocketServerCore::WebSocketServerCore(const String & origin, const String & protocol) {
    _origin                 = origin;
    _protocol               = protocol;
    _runnning               = false;
    _pingInterval           = 0;
...
WebSocketServer::WebSocketServer(uint16_t port, const String & origin, const String & protocol)
    : WebSocketServerCore(origin, protocol) {
    _port = port;
    ...
    _server->onClient([](void * s, AsyncClient * c) {
        ((WebSocketServerCore *)s)->newClient(new AsyncTCPbuffer(c));
    },
        this);
#endif
}

WebSocketServerCore::~WebSocketServerCore() {
 ...
 */
void WebSocketServerCore::begin(void) {
...
void WebSocketServerCore::close(void) {
    _runnning = false;
    disconnect();
 ...
 */
void WebSocketServerCore::onEvent(WebSocketServerEvent cbEvent) {
    _cbEvent = cbEvent;
}
 ...
 */
void WebSocketServerCore::onValidateHttpHeader(
 ...
 * @return true if ok
 */
bool WebSocketServerCore::sendTXT(uint8_t num, uint8_t * payload, size_t length, bool headerToPayload) {
...
bool WebSocketServerCore::sendTXT(uint8_t num, const uint8_t * payload, size_t length) {
    return sendTXT(num, (uint8_t *)payload, length);
}

bool WebSocketServerCore::sendTXT(uint8_t num, char * payload, size_t length, bool headerToPayload) {
    return sendTXT(num, (uint8_t *)payload, length, headerToPayload);
}

bool WebSocketServerCore::sendTXT(uint8_t num, const char * payload, size_t length) {
    return sendTXT(num, (uint8_t *)payload, length);
}

bool WebSocketServerCore::sendTXT(uint8_t num, String & payload) {
 ...
 * @return true if ok
 */
bool WebSocketServerCore::broadcastTXT(uint8_t * payload, size_t length, bool headerToPayload) {
    WSclient_t * client;
    bool ret = true;
    if(length == 0) {
        length = strlen((const char *)payload);
...
bool WebSocketServerCore::broadcastTXT(const uint8_t * payload, size_t length) {
    return broadcastTXT((uint8_t *)payload, length);
}

bool WebSocketServerCore::broadcastTXT(char * payload, size_t length, bool headerToPayload) {
    return broadcastTXT((uint8_t *)payload, length, headerToPayload);
}

bool WebSocketServerCore::broadcastTXT(const char * payload, size_t length) {
    return broadcastTXT((uint8_t *)payload, length);
}

bool WebSocketServerCore::broadcastTXT(String & payload) {
 ...
 * @return true if ok
 */
bool WebSocketServerCore::sendBIN(uint8_t num, uint8_t * payload, size_t length, bool headerToPayload) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return false;
    }
    WSclient_t * client = &_clients[num];
...
bool WebSocketServerCore::sendBIN(uint8_t num, const uint8_t * payload, size_t length) {
 ...
 * @return true if ok
 */
bool WebSocketServerCore::broadcastBIN(uint8_t * payload, size_t length, bool headerToPayload) {
    WSclient_t * client;
    bool ret = true;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
...
bool WebSocketServerCore::broadcastBIN(const uint8_t * payload, size_t length) {
 ...
 */
bool WebSocketServerCore::sendPing(uint8_t num, uint8_t * payload, size_t length) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return false;
    }
    WSclient_t * client = &_clients[num];
...
bool WebSocketServerCore::sendPing(uint8_t num, String & payload) {
 ...
 * @return true if ping is send out
 */
bool WebSocketServerCore::broadcastPing(uint8_t * payload, size_t length) {
    WSclient_t * client;
    bool ret = true;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
...
bool WebSocketServerCore::broadcastPing(String & payload) {
 ...
 * disconnect all clients
 */
void WebSocketServerCore::disconnect(void) {
    WSclient_t * client;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
        if(clientIsConnected(client)) {
 ...
 * @param num uint8_t client id
 */
void WebSocketServerCore::disconnect(uint8_t num) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return;
    }
    WSclient_t * client = &_clients[num];
 ...
 */
void WebSocketServerCore::setAuthorization(const char * user, const char * password) {
    if(user && password) {
        String auth = user;
        auth += ":";
        auth += password;
 ...
 */
void WebSocketServerCore::setAuthorization(const char * auth) {
    if(auth) {
 ...
 */
int WebSocketServerCore::connectedClients(bool ping) {
    WSclient_t * client;
    int count = 0;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
 ...
 * @param num uint8_t client id
 */
bool WebSocketServerCore::clientIsConnected(uint8_t num) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return false;
    }
    WSclient_t * client = &_clients[num];
 ...
 * @return IPAddress
 */
IPAddress WebSocketServerCore::remoteIP(uint8_t num) {
    if(num < WEBSOCKETS_SERVER_CLIENT_MAX) {
        WSclient_t * client = &_clients[num];
        if(clientIsConnected(client)) {
 ...
 * @param client
 */
WSclient_t * WebSocketServerCore::newClient(WEBSOCKETS_NETWORK_CLASS * TCPclient) {
    WSclient_t * client;
    ...
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        ...
        if(clientIsConnected(client)) {
...
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
            client->tcp->onDisconnect(std::bind([](WebSocketServerCore * server, AsyncTCPbuffer * obj, WSclient_t * client) -> bool {
            ...
            client->tcp->readStringUntil('\n', &(client->cHttpLine), std::bind(&WebSocketServerCore::handleHeader, this, client, &(client->cHttpLine)));
#endif
 ...
 * @param length size_t
 */
void WebSocketServerCore::messageReceived(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin) {
 ...
 */
void WebSocketServerCore::dropNativeClient(WSclient_t * client) {
    if(!client) {
        return;
    }
    if(client->tcp) {
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void WebSocketServerCore::clientDisconnect(WSclient_t * client) {
 ...
 */
bool WebSocketServerCore::clientIsConnected(WSclient_t * client) {
    if(!client->tcp) {
        return false;
    }
 ...
 */
WSclient_t * WebSocketServerCore::handleNewClient(WEBSOCKETS_NETWORK_CLASS * tcpClient) {
 ...
 * Handle incoming Connection Request
 */
void WebSocketServer::handleNewClients(void) {
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266) || (WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP32) || (WEBSOCKETS_NETWORK_TYPE == NETWORK_RP2040)
 ...
 */
void WebSocketServerCore::handleClientData(void) {
    WSclient_t * client;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
        if(clientIsConnected(client)) {
 ...
 */
bool WebSocketServerCore::hasMandatoryHeader(String headerName) {
    for(size_t i = 0; i < _mandatoryHttpHeaderCount; i++) {
 ...
 */
void WebSocketServerCore::handleHeader(WSclient_t * client, String * headerLine) {
    ...
    if(headerLine->length() > 0) {
        ...
        if(ok) {
            ...
            String handshake = WEBSOCKETS_STRING(
                ...
                "Server: arduino-WebSocketServer\r\n"
                "Upgrade: websocket\r\n"
                "Connection: Upgrade\r\n"
 ...
 */
void WebSocketServerCore::handleHBPing(WSclient_t * client) {
 ...
 */
void WebSocketServerCore::enableHeartbeat(uint32_t pingInterval, uint32_t pongTimeout, uint8_t disconnectTimeoutCount) {
 ...
 */
void WebSocketServerCore::disableHeartbeat() {
    _pingInterval = 0;
 ...
 * called to initialize the Websocket server
 */
void WebSocketServer::begin(void) {
    WebSocketServerCore::begin();
    _server->begin();

    DEBUG_WEBSOCKETS("[WS-Server] Server Started.\n");
}

void WebSocketServer::close(void) {
    WebSocketServerCore::close();
 ...
 */
void WebSocketServerCore::loop(void) {
    if(_runnning) {
        WEBSOCKETS_YIELD();
        handleClientData();
    }
 ...
 * called in arduino loop
 */
void WebSocketServer::loop(void) {
    if(_runnning) {
        WEBSOCKETS_YIELD();
        handleNewClients();
        WebSocketServerCore::loop();
    }
}
#endif
