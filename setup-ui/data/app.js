...
function getWiFiList () {
  ...
  const url = new URL(`${window.location.protocol}//${window.location.hostname}/scan`)
  fetch(url)
...
function listWifiNetworks (elems) {
  ...
  elems.forEach((elem, idx) => {
    ...
    if (elem.security) { row.innerHTML += '<td>' + svgLock + '</td>' } else { row.innerHTML += '<td>' + svgUnlock + '</td>' }
...
function getEspStatus () {
  const url = new URL(`${window.location.protocol}//${window.location.hostname}/status`)
  fetch(url)
    .then(response => response.json())
    .then(data => {
      $('esp-mode').innerHTML = data.mode
      $('esp-ssid').innerHTML = data.ssid
...
function getParameters () {
  $('loader').classList.remove('hide')
  const url = new URL(`${window.location.protocol}//${window.location.hostname}/setup/config.json`)
  fetch(url)
    .then(response => response.json())
    .then(data => {
  ...
  Object.entries(params).forEach(([key, value], i) => {
    if (key.startsWith('name-logo')) {
      ...
      if (typeof (val) === 'boolean') {
        ...
        lbl.textContent = key
        if (typeof (val) === 'number') { el.setAttribute('type', 'number') }
        ...
        if (typeof (val) === 'object') {
          ...
          // This is a float value
            ...
            const num = Math.round(val.value * (1 / val.step)) / (1 / val.step)
...
function saveParameters () {
  const myblob = new Blob([JSON.stringify(options, null, 2)], {
  ...
  // Handle the server response
    ...
    .then(() => {
      openModalMessage('Save options', '<br><b>config.json</b> file saved successfully on flash memory!<br><br>')
    })
}
...
function switchPage (el) {
  ...
  } else { $('btn-box').classList.add('hide') }
}
...
function openModalMessage (title, msg, fn) {
  ...
  } else { $('ok-modal').classList.add('hide') }
}

function closeModalMessage (doCb) {
  $('modal-message').open = false
  $('main-box').style.filter = ''
  if (typeof closeCallback !== 'undefined' && doCb) { closeCallback() }
}

function restartESP () {
  const url = new URL(`${window.location.protocol}//${window.location.hostname}/restart`)
  fetch(url)
    .then(response => response.text())
    .then(() => {
      closeModalMessage()
...
$('password').addEventListener('input', (event) => {
  if ($('password').value.length === 0) { $('connect-wifi').disabled = true } else { $('connect-wifi').disabled = false }
})
